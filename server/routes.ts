...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/monde/tarefas/:id", authenticateToken, async (req: any, res) => {
    try {
      const taskId = req.params.id;
      const { person_id, title, category_id, assignee_id, due_date, completed, priority } = req.body;
      
      console.log('Atualizando tarefa ID:', taskId, 'com dados:', req.body);
      
      const mondeUrl = `https://web.monde.com.br/api/v2/tasks/${taskId}`;
      
      // Construir o corpo da requisição seguindo o formato da API do Monde
      ...
      if (title !== undefined) {
        requestBody.data.attributes.title = title;
      }
      if (due_date !== undefined) {
        requestBody.data.attributes.due = due_date;
      }
      if (completed !== undefined) {
        requestBody.data.attributes.completed = completed;
      }
      
      // Adicionar relationships se fornecidos
      if (person_id || category_id || assignee_id) {
        requestBody.data.relationships = {};
        
        if (person_id) {
          requestBody.data.relationships.person = {
            data: {
              type: "people",
              id: person_id
            }
        ...
        if (category_id) {
          requestBody.data.relationships.category = {
            data: {
              type: "task-categories",
              id: category_id
            }
        ...
        if (assignee_id) {
          requestBody.data.relationships.assignee = {
            data: {
              type: "people",
              id: assignee_id
            }
      ...
      console.log('Request body para atualizar tarefa:', JSON.stringify(requestBody, null, 2));
      ...
      const responseText = await mondeResponse.text();
      console.log('Response status da atualização:', mondeResponse.status);
      console.log('Response body da atualização:', responseText);
